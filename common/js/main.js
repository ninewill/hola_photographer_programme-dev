  ...
  var load = function () {
    //首頁
		...
		if ($(window).width() > 1024) {
			gsap.to(".loading-txt", 0.3, {
				delay: 0.1,
	
				onComplete: function () {
					gsap.to($(".loading-txt"), 0.5, {
						css: {
							fontSize: "96px",
						},
						ease: Expo.easeInOut,
    ...
    gsap.to(".loadin-gpage", 0.5, {
      delay: 1,
      opacity: 0,

      onComplete: function () {
        gsap.to($(".loadin-gpage"), 0.1, {
          css: {
            display: "none",
          },
        });
...
$(function () {
   ...
   */
	...
	//電腦版
  $(".program-btn").on("click", function (event) {
  ...
  $(document).keyup(function (event) {
	...
	//電腦版2
	$(".program-btn2").on("click", function (event) {
    event.preventDefault();
    $(".cd-popup").addClass("is-visible");
  });
  ...
  $(document).keyup(function (event) {
	...
	//手機版
	$(".program-btn-mobile").on("click", function (event) {
    event.preventDefault();
    $(".cd-popup-mobile").addClass("is-visible");
  });

  //close popup
  $(".cd-popup-mobile").on("click", function (event) {
    if (
      $(event.target).is(".cd-popup-close-mobile") ||
      $(event.target).is(".cd-popup-mobile")
    ) {
      event.preventDefault();
      $(this).removeClass("is-visible");
    }
  ...
  $(document).keyup(function (event) {
    if (event.which == "27") {
      $(".cd-popup-mobile").removeClass("is-visible");
    }
  });
   ...
   */
	...
	//電腦版
  let popupswiper = new Swiper(".popup-swiper", {
    navigation: {
	...
	let popupswiper1 = new Swiper(".popup-swiper1", {
    navigation: {
	...
	let popupswiper2 = new Swiper(".popup-swiper2", {
    navigation: {
	...
	let s2carousel = new Swiper(".s2-carousel", {
		loop: true,
		autoplay: {
      delay: 3000,
  ...
  let tabLink = $(".tab-titel");

  let tabContents = $(".form-content");
